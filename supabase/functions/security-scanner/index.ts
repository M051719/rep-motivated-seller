...
const SECURITY_PATTERNS = {
  secrets: [
    {
      ...
      cwe: "CWE-798",
      remediation:
        "Remove hardcoded Stripe live keys. Use environment variables instead.",
    },
    {
      ...
      cwe: "CWE-798",
      remediation:
        "Remove hardcoded Stripe test keys. Use environment variables instead.",
    },
    {
      ...
      cwe: "CWE-798",
      remediation:
        "Verify if this is an API key and move to environment variables.",
    },
    {
      ...
      cwe: "CWE-259",
      remediation:
        "Remove hardcoded passwords. Use secure environment variables.",
    },
    {
      pattern:
        /(?:secret|api_?key|private_?key)\s*[:=]\s*(['"`])(?!.*\$\{|.*process\.env|.*Deno\.env).+?\1/gi,
  ...
  injection: [
    {
      ...
      cwe: "CWE-95",
      remediation:
        "Avoid using eval(). Use safer alternatives like JSON.parse() or specific parsers.",
    },
    {
      ...
      cwe: "CWE-95",
      remediation:
        "Avoid dynamic function creation. Use predefined functions instead.",
    },
    {
      ...
      cwe: "CWE-79",
      remediation:
        "Use safer DOM manipulation methods like textContent or innerHTML with sanitization.",
    },
    {
      ...
      cwe: "CWE-79",
      remediation:
        "Sanitize user input before using innerHTML or use textContent instead.",
    },
  ],
  permissions: [
    {
      ...
      cwe: "CWE-250",
      remediation:
        "Ensure service role usage is necessary and properly secured. Consider using RLS policies instead.",
    },
    {
      ...
      cwe: "CWE-250",
      remediation:
        "Verify that elevated privileges are necessary and properly controlled.",
    },
    {
      ...
      cwe: "CWE-285",
      remediation:
        "Review RLS bypass usage. Ensure it's intentional and secure.",
    },
  ],
  dataExposure: [
    {
      pattern:
        /console\.log.*password|console\.log.*secret|console\.log.*key/gi,
  ...
  cryptography: [
    ...
    },
    {
      ...
      cwe: "CWE-327",
      remediation:
        "Use stronger cryptographic algorithms like SHA-256 or bcrypt for passwords.",
    },
  ],
};
...
function scanFunctionCode(code: string, scanType: string): SecurityIssue[] {
  ...
  for (const category of patternCategories) {
    const patterns =
      SECURITY_PATTERNS[category as keyof typeof SECURITY_PATTERNS];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: functionData, error: functionError } =
      await supabaseAdminClient.rpc("get_edge_function_source", {
    ...
    if (functionError) {
      ...
      throw new Error(
        `Failed to fetch function source: ${functionError.message}`,
      );
    }
