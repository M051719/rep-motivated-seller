...
interface ComplianceLogEntry {
  event_type: 'data_access' | 'payment_processing' | 'privacy_request' | 'data_retention' | 'security_incident'
  user_id?: string
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`📊 Logging compliance event: ${logEntry.event_type} - ${logEntry.action}`)

    // Validate required fields
    if (!logEntry.event_type || !logEntry.resource_type || !logEntry.action) {
      throw new Error('Missing required fields: event_type, resource_type, action')
    }
    ...
    const { error: schemaError } = await supabaseClient.rpc('create_compliance_log_table')
    if (schemaError) {
    ...
    if (logEntry.event_type === 'security_incident' || 
