  ...
  try {
    ...
    const successResponse = {
      ...
      hubspot_response: {
        ...
        sample_contact: hubspotData.results && hubspotData.results.length > 0 ? {
          id: hubspotData.results[0].id,
          email: hubspotData.results[0].properties?.email || 'N/A',
          name: `${hubspotData.results[0].properties?.firstname || ''} ${hubspotData.results[0].properties?.lastname || ''}`.trim() || 'N/A'
        } : 'No contacts found'
      }
    }
