    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return new Response(
        JSON.stringify({ 
          ...
          message: 'This endpoint requires a valid JWT token. Please include "Authorization: Bearer q6djVCdScS2VQeLx5EG+8jBqvw7Ox8g9RCcE8UGZUTewCYmpYmgDkUlDzh3/9dbNCx00U/xyOj0e+HvLdjGiUA==" in your request headers.'
        }),
        {
          status: 401,
    ...
    if (req.method === 'GET') {
      ...
      if (path === 'stats') {
        ...
        const { data: statusCounts, error: statusError } = await supabaseClient
          .rpc('get_submission_counts_by_status')

        // Get counts by assigned admin
        const { data: assignedCounts, error: assignedError } = await supabaseClient
          .rpc('get_submission_counts_by_admin')
        ...
        if (statusError || assignedError || activityError) {
          return new Response(
            JSON.stringify({ 
              error: statusError?.message || assignedError?.message || activityError?.message 
            }),
            {
              status: 400,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
