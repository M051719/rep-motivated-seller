...
serve(async (req) => {
  ...
  try {
    ...
    for (const [index, recipient] of recipients.entries()) {
      try {
        console.log(`📤 Processing recipient ${index + 1}/${recipients.length}: ${recipient.name}`)
        ...
        const lobResponse = await fetch('https://api.lob.com/v1/postcards', {
          ...
          body: JSON.stringify({
            ...
            from: senderAddress,
            front: templateUrl || '<html><body style="background: linear-gradient(45deg, #667eea, #764ba2); color: white; text-align: center; padding: 50px;"><h1 style="font-size: 24px;">🏠 RepMotivatedSeller</h1><p style="font-size: 16px;">Foreclosure Prevention Experts</p></body></html>',
            back: postcardBack,
            size: '4x6',
            mail_type: mailType,
            color: color,
        ...
        if (lobResponse.ok) {
          ...
          sentCount++
          const cost = parseFloat(lobResult.expected_delivery_date ? '0.50' : '0.50') // Approximate cost
          totalCost += cost
          ...
          console.error(`❌ Lob API error for ${recipient.name}:`, lobResponse.status, errorData)
    ...
    console.log(`📊 Campaign results: ${sentCount} sent, ${failedCount} failed, $${totalCost.toFixed(2)} total cost`)
