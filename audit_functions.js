...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx0eHFvZHFsZXh2b2pxcXhxdWV3Iiwicm9sZUEibmFub24iLCJpYXQiOjE3MzUxNDY3MjksImV4cCI6MjA1MDcyMjcyOX0.sb_publishable_wQE2WLlEvqE1RqWtNPcxGw_tSpNMwmg'
...
async function auditAllFunctions() {
  ...
  for (const [category, functions] of Object.entries(functionCategories)) {
    ...
    for (const functionName of functions) {
      ...
      console.log(`  ${result.accessible ? '✅' : '❌'} ${functionName} (${result.status})`)
    }
    console.log('')
  }
  ...
  console.log(`Accessible: ${results.accessible} (${Math.round(results.accessible/results.total*100)}%)`)
  ...
  for (const [category, data] of Object.entries(results.categories)) {
    ...
    console.log(`${category.toUpperCase()}: ${data.accessible}/${data.total} (${successRate}%)`)
  }
