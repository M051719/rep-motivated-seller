...
const SchedulingDashboard: React.FC = () => {
  ...
  const calendlyLinks = {
    ...
    entrepreneur: 'https://calendly.com/repmotivatedseller/entrepreneur-strategy-session',
    professional: 'https://calendly.com/repmotivatedseller/professional-deep-dive',
    enterprise: 'https://calendly.com/repmotivatedseller/enterprise-consultation'
  };
  ...
  const getEventTypeIcon = (type: '1-on-1' | 'Zoom') => {
    return type === 'Zoom' ? <Video className="w-5 h-5" /> : <Users className="w-5 h-5" />;
  };
  ...
  const getTierColor = (tier: string) => {
    switch (tier) {
      ...
      case 'entrepreneur': return 'text-blue-600';
    ...
    switch (tier) {
      case 'free':
        return {
          ...
          features: ['Basic foreclosure guidance', 'Resource recommendations', 'Q&A session']
        };
      case 'entrepreneur':
        return {
          ...
          duration: '45 minutes',
          type: '1-on-1 or Zoom',
          features: ['Strategy planning', 'Deal analysis review', 'Action plan creation', 'Email follow-up']
        };
      case 'professional':
        return {
          ...
          duration: '60 minutes',
          type: '1-on-1 or Zoom',
          features: ['Deep-dive consultations', 'Custom deal analysis', 'Contract review', 'Priority scheduling', 'Direct phone access']
        };
      case 'enterprise':
        return {
          ...
          features: ['Dedicated account manager', 'Same-day scheduling', 'Custom consultation packages', 'Team training sessions', '24/7 priority support']
        };
      default:
        return {
          sessions: 'N/A',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Manage your appointments and consultations with foreclosure prevention experts
          </p>
        </motion.div>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-3xl font-bold mb-2">
                Your {userTier.charAt(0).toUpperCase() + userTier.slice(1)} Plan Benefits
              </h2>
              <p className="text-blue-100">Access to expert consultation and guidance</p>
            </div>
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            ...
            >
              ...
              <p className="text-gray-600 mb-6">Schedule your first consultation to get expert guidance</p>
              <button
                onClick={() => handleScheduleNew(userTier)}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-3 rounded-full ${
                        appointment.type === 'Zoom' ? 'bg-purple-100' : 'bg-blue-100'
                      }`}>
                        {getEventTypeIcon(appointment.type)}
                      </div>
                      <div>
                        <h3 className="font-bold text-gray-900">{appointment.title}</h3>
                        <p className={`text-sm font-medium ${getTierColor(appointment.tier)}`}>
        ...
        >
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Schedule by Tier</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {Object.entries(calendlyLinks).map(([tier, url]) => (
              <div
                key={tier}
                className={`border-2 rounded-xl p-6 ${
                  userTier === tier ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                }`}
              >
