...
interface SMSConversation {
  ...
  phone_number: string;
  contact_name?: string;
  contact_type: 'real_estate_professional' | 'investor' | 'prospect' | 'client' | 'unknown';
...
const AdminSMSDashboard = () => {
  ...
  const loadConversations = async () => {
    try {
      ...
      if (searchQuery) {
        query = query.or(`phone_number.ilike.%${searchQuery}%,contact_name.ilike.%${searchQuery}%`);
      }
  ...
  const useQuickReply = (reply: QuickReply) => {
    ...
    if (selectedConversation) {
      ...
      message = message.replace('{name}', selectedConversation.contact_name || 'there');
    }
  ...
  const getContactTypeIcon = (type: string) => {
    switch (type) {
      case 'real_estate_professional': return '🏢';
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-[1800px] mx-auto px-4 py-6">
          ...
          {/* Header */}
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">SMS Monitoring Dashboard</h1>
            <p className="text-gray-600">
              Monitor and respond to SMS conversations • {getTotalUnread()} unread messages
            </p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
            <div className="grid md:grid-cols-4 gap-4">
              ...
              <div>
                <select
                  value={filterStatus}
                  onChange={(e) => { setFilterStatus(e.target.value); loadConversations(); }}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div>
                <select
                  ...
                  onChange={(e) => { setFilterContactType(e.target.value); loadConversations(); }}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  <option value="all">All Contact Types</option>
                  <option value="real_estate_professional">Real Estate Professionals</option>
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            <span className="text-2xl">{getContactTypeIcon(conv.contact_type)}</span>
                            <div>
                              <p className="font-semibold text-gray-900">
                                {conv.contact_name || formatPhoneNumber(conv.phone_number)}
                              </p>
                              {conv.contact_name && (
                                <p className="text-xs text-gray-500">{formatPhoneNumber(conv.phone_number)}</p>
                              )}
                            </div>
                          </div>
                          {conv.unread_count > 0 && (
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <div className="flex items-center space-x-2">
                            <span className={`px-2 py-1 rounded-full ${getStatusColor(conv.status)}`}>
                              {conv.status}
                            </span>
                          ...
                          <span className="text-gray-500">
                            {conv.last_message_at ? formatTimestamp(conv.last_message_at) : formatTimestamp(conv.created_at)}
                          </span>
                        </div>
                  ...
                  <div className="p-4 border-b border-gray-200">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {getContactTypeIcon(selectedConversation.contact_type)} {selectedConversation.contact_type.replace(/_/g, ' ')} •{' '}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) => updateConversation({ contact_type: e.target.value as any })}
                        className="px-3 py-2 border border-gray-300 rounded text-sm"
                      >
                        <option value="unknown">Unknown</option>
                        <option value="real_estate_professional">Professional</option>
                      ...
                      >
                        ...
                        <option value="foreclosure_assistance">Foreclosure Help</option>
