...
export const runEnhancedDiagnostics = async (): Promise<DiagnosticResult> => {
  ...
  console.log(
    `   URL: ${envUrl ? "✅ Set" : "❌ Missing"} ${envUrl ? `(${envUrl.substring(0, 30)}...)` : ""}`,
  );
  console.log(
    `   Key: ${envKey ? "✅ Set" : "❌ Missing"} ${envKey ? `(${envKey.substring(0, 20)}...)` : ""}`,
  );
  console.log(
    `   URL Format: ${envUrl?.startsWith("http") ? "✅ Valid" : "❌ Invalid"}\n`,
  );
  ...
  if (result.auth.success) {
    ...
    console.log(
      `   User: ${result.auth.authenticated ? "✅ Authenticated" : "✅ Anonymous"}`,
    );
    if (result.auth.user) {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "PGRST301") {
        console.log(
          `   🔍 This is normal if no RLS policy allows anonymous access`,
        );
      ...
      console.log(
        `   RLS Test: ✅ Accessible (returned ${data?.length || 0} rows)`,
      );
    }
  } catch (error: any) {
    result.rls = { accessible: false, error: error.message };
