...
class FreePropertyIntelligence {
  ...
  async getForeclosureListings(state: string, county: string) {
    const sources = {
      ...
      HUD: async () => {
        const response = await fetch('https://www.hudhomestore.gov/Home/AjaxListingSearch', {
          method: 'POST',
      ...
      FANNIE_MAE: async () => {
        const response = await fetch(`https://www.homepath.com/api/v1/listings?state=${state}&limit=500`);
        return response.json();
      },

      // Freddie Mac HomeSteps (FREE)
      FREDDIE_MAC: async () => {
        const response = await fetch(`https://www.homesteps.com/api/properties/search?state=${state}`);
        return response.json();
      },
      ...
      USDA: async () => {
        const response = await fetch(`https://properties.sc.egov.usda.gov/resales/api/properties/search?state=${state}`);
        return response.json();
      }
    };
  ...
  async getTaxDelinquentProperties(county: string, state: string) {
    ...
    if (!siteInfo) {
      return { message: `Manual search required at ${county} County Treasurer website` };
    }
  ...
  async findAbsenteeOwners(zipCode: string) {
    ...
    const strategies = {
      ...
      vacancyData: async () => {
        ...
        const response = await fetch(`https://www.huduser.gov/portal/datasets/usps/forVacancyData.html`);
        return {
    ...
    const signals = {
      ...
      craigslist: async () => {
        ...
        return {
          ...
          keywords: ['must sell', 'motivated', 'as-is', 'cash only', 'quick sale']
        };
      },
