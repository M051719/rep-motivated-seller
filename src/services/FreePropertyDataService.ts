...
class FreePropertyDataService {
  ...
  async getGoogleMapsData(address: string) {
    try {
      ...
      return {
        ...
        streetViewUrl: streetViewData.status === 'OK' 
  ...
  async getOpenStreetMapData(address: string) {
    try {
      ...
      const searchResponse = await fetch(
        ...
        {
          headers: {
            'User-Agent': 'RepMotivatedSeller/1.0 (contact@repmotivatedseller.org)'
          }
        }
      );
  ...
  async estimatePropertyValue(address: string, propertyData: any) {
    try {
      ...
      const censusMedianValue = propertyData?.census?.demographics?.medianHomeValue;
      ...
      // Apply adjustments
      estimatedValue = estimatedValue * adjustments.propertyType * adjustments.neighborhood * adjustments.marketTrend;
  ...
  async getForeclosureData(address: string, county: string, state: string) {
    ...
    const foreclosureSites: Record<string, string> = {
      ...
      'miami-dade-fl': 'https://www.miami-dadeclerk.com/official-records/search-instruments.page',
      // Add more counties
    };
    ...
    return {
      ...
      source: foreclosureSites[`${county.toLowerCase()}-${state.toLowerCase()}`] || 'Not available'
    };
  }
  ...
  async getComprehensivePropertyData(address: string) {
    ...
    const [googleData, censusData, osmData, countyData] = await Promise.all([
      ...
      county ? this.getCountyAssessorData(address, county, state).catch(err => {
    ...
    const combinedData = {
      ...
      coordinates: googleData?.coordinates || osmData?.coordinates || censusData?.coordinates,
      
      // Property details
      property: {
        type: googleData?.propertyType?.[0] || osmData?.placeType || 'residential',
      ...
      neighborhood: {
        ...
        medianIncome: censusData?.censusData?.demographics?.medianHouseholdIncome,
