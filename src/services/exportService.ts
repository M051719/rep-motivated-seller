...
export async function generatePDF(data: PresentationData): Promise<Blob> {
  ...
  addText(
    `Bedrooms: ${data.property.bedrooms} | Bathrooms: ${data.property.bathrooms}`,
    12,
  );
  ...
  if (data.property.monthlyPayment) {
    addText(
      `Monthly Payment: $${data.property.monthlyPayment.toLocaleString()}`,
      12,
    );
  }
  ...
  if (data.comparables.length > 0) {
    data.comparables.forEach((comp, index) => {
      ...
      addText(
        `   Price: $${comp.price.toLocaleString()} | ${comp.bedrooms}bd/${comp.bathrooms}ba | ${comp.sqft.toLocaleString()} sqft`,
        11,
      );
      addText(
        `   Sold: ${comp.soldDate} | ${comp.distance.toFixed(1)} miles away | $${comp.pricePerSqft}/sqft`,
        11,
      );
      yPosition += 2;
    });
    ...
    // Calculate average
    const avgPrice =
      data.comparables.reduce((sum, c) => sum + c.price, 0) /
      data.comparables.length;
    const avgPricePerSqft =
      data.comparables.reduce((sum, c) => sum + c.pricePerSqft, 0) /
      data.comparables.length;
...
export async function generatePPTX(data: PresentationData): Promise<Blob> {
  ...
  if (data.property.equity) {
    propertyDetails.push([
      "Equity:",
      `$${data.property.equity.toLocaleString()}`,
    ]);
  }
  ...
  if (data.comparables.length > 0) {
    const compTable = [["Address", "Price", "Beds/Baths", "Sqft", "Distance"]];
  ...
  if (data.aiContent && data.aiContent.marketingSummary) {
    ...
    if (data.aiContent.keyFeatures && data.aiContent.keyFeatures.length > 0) {
      const features = data.aiContent.keyFeatures
        .map((f) => `• ${f}`)
        .join("\n");
...
export async function sendPresentationEmail(
  ...
  try {
    ...
    return {
      success: false,
      message:
        error instanceof Error ? error.message : "Failed to send presentation",
    };
  }
}
