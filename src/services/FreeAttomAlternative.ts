...
class FreeAttomAlternative {
  ...
  async getParcelData(address: string) {
    ...
    for (const state of Object.values(gisServers)) {
      for (const county of state) {
        try {
          const response = await fetch(`${county.url}?${new URLSearchParams(county.params as any)}`);
  ...
  private async getRedfinSales(address: string) {
    ...
    try {
      ...
      const response = await fetch(`https://www.redfin.com/stingray/api/home/details/aboveTheFold?path=/address/${encodeURIComponent(address)}`);
      const data = await response.json();
  ...
  async getMortgageData(address: string) {
    ...
    const secData = await fetch('https://www.sec.gov/edgar/searchedgar/companysearch.html');
  ...
  async getPropertyCharacteristics(address: string) {
    ...
    return {
      ...
      squareFootage: sources.assessorCard?.squareFootage || sources.gisData?.SQFT,
  ...
  async getSchoolData(address: string) {
    ...
    const ncesData = await fetch(`https://nces.ed.gov/ccd/api/schools?address=${encodeURIComponent(address)}`);
  ...
  private async getCensusData(address: string) {
    ...
    const response = await fetch(`https://api.census.gov/data/2021/acs/acs5?get=B01003_001E,B19013_001E,B25077_001E&for=tract:*&in=state:*`);
  ...
  private async getWalkabilityScore(address: string) {
    ...
    const osmData = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(address)}&format=json`);
  ...
  private estimateValue(data: any[]) {
    ...
    const avgPricePerSqft = recentSales.reduce((sum: number, sale: any) => 
      sum + (sale.price / sale.sqft), 0) / recentSales.length || 200;
  ...
  private mergeSalesData(sources: any[]) {
    ...
    const uniqueSales = Array.from(new Map(allSales.map(s => [s.date, s])).values());
