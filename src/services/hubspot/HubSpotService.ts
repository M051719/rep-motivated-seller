...
export class HubSpotService {
  ...
  static async validateConnection(): Promise<ConnectionTest[]> {
    ...
    try {
      const { data, error } = await supabase.rpc("validate_hubspot_connection");
  ...
  static async syncSubmission(submissionId: string): Promise<SyncResult> {
    ...
    try {
      const { data, error } = await supabase.rpc("sync_submission_to_hubspot", {
        submission_id: submissionId,
      });
  ...
  static async bulkSync(limit: number = 10): Promise<BulkSyncResult> {
    ...
    try {
      const { data, error } = await supabase.rpc("bulk_sync_to_hubspot", {
        limit_count: limit,
      });
