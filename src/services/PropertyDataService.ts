...
class PropertyDataService {
  ...
  async getAttomData(address: string) {
    const response = await fetch(`https://api.gateway.attomdata.com/propertyapi/v1.0.0/property/detail`, {
      headers: {
  ...
  private async getCensusData(address: string) {
    ...
    const response = await fetch(`https://geocoding.geo.census.gov/geocoder/locations/onelineaddress?address=${encodeURIComponent(address)}&benchmark=2020&format=json`);
    return response.json();
  }
}
