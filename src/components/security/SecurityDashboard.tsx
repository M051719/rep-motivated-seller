...
const SecurityDashboard: React.FC = () => {
  ...
  const performSecurityChecks = async () => {
    ...
    const checks: SecurityCheck[] = [
      ...
      },
      {
        ...
        status: document.querySelector('meta[http-equiv="X-XSS-Protection"]')
          ? "pass"
          : "fail",
  ...
  const getOverallScore = () => {
    ...
    const passed = securityChecks.filter(
      (check) => check.status === "pass",
    ).length;
    const warnings = securityChecks.filter(
    ...
    <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-sm text-gray-500">{score.total} checks</div>
            </div>
          </div>
        </div>
      </motion.div>
          ...
          >
            <div className="p-6">
              <div className="flex items-start justify-between">
                ...
                <div
                  className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(check.status)}`}
                >
      ...
      >
        ...
        <div className="space-y-2 text-sm">
          <p>
            • Run automated security scans with{" "}
            <code>npm run security:scan</code>
          </p>
          <p>
            • Check for vulnerabilities in dependencies with{" "}
            <code>npm audit</code>
          </p>
