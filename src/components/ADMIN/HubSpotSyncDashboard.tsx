...
import {
  ...
  SyncStats,
  SyncError,
} from "../../services/hubspot/HubSpotService";
import {
  RefreshCw,
  CheckCircle,
  XCircle,
...
export const HubSpotSyncDashboard: React.FC = () => {
  ...
  const handleResolveError = async (errorId: string) => {
    ...
    const success = await HubSpotService.resolveError(
      errorId,
      notes || undefined,
    );
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        {unsyncedCount > 0 && (
          <p className="mt-4 text-sm text-gray-600">
            {unsyncedCount} unsynced submission{unsyncedCount !== 1 ? "s" : ""}{" "}
            ready to sync
          </p>
        )}
      </div>
                ...
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      {new Date(error.created_at).toLocaleString()} • Attempts:{" "}
                      {error.retry_count}
                    </p>
                  </div>
                  <button
