...
export const SMSConsentManager: React.FC<SMSConsentManagerProps> = ({
  ...
  const [consentStatus, setConsentStatus] = useState<SMSConsentStatus | null>(null)
  ...
  const checkConsentStatus = async (phone: string) => {
    ...
    setLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('sms-consent/status', {
  ...
  const handleOptIn = async () => {
    ...
    setLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('sms-consent/opt-in', {
        body: {
  ...
  const handleOptOut = async () => {
    ...
    setLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('sms-consent/opt-out', {
        body: {
          phone_number: phoneNumber,
          method: 'web_request',
  ...
  const loadConsentHistory = async () => {
    ...
    setLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('sms-consent/history', {
    ...
    <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Manage your SMS notification preferences. You can opt in or out at any time.
        </p>
      </div>
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <p className="text-sm text-blue-900">
          <strong>📱 SMS Terms:</strong> By opting in, you agree to receive SMS messages from
          RepMotivatedSeller. Message frequency varies. Message and data rates may apply. Reply
          ...
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[80vh] overflow-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-900">Consent History</h3>
                <button
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            ...
                            {!event.previous_status && <span>{event.new_status}</span>}
                          </p>
                          {event.method && (
                          ...
                          {event.keyword_received && (
                            <p className="text-sm text-gray-500">Keyword: {event.keyword_received}</p>
                          )}
                        </div>
