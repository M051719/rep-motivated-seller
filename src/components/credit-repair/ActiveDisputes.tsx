  ...
  if (disputes.length === 0) {
    return (
      <div className="no-disputes">
        <p>No active disputes. Start improving your credit by filing a dispute!</p>
        <button 
          ...
          <div key={dispute.id} className="dispute-item">
            <div className="dispute-header">
              <div>
                ...
                <p className="dispute-creditor">{dispute.creditor} • {dispute.account}</p>
              </div>
              <span className={`dispute-status ${getStatusClass(dispute.status)}`}>
                {dispute.status}
              </span>
            </div>
            ...
            <div className="dispute-details">
              <div className="detail-item">
                ...
                <span className="detail-value">{formatDate(dispute.submittedDate)}</span>
              </div>
              
              {dispute.status !== 'Resolved' && dispute.expectedResolution && (
                <div className="detail-item">
                  <span className="detail-label">Expected Resolution:</span>
                  <span className="detail-value">{formatDate(dispute.expectedResolution)}</span>
                </div>
              )}
              ...
              {dispute.status === 'Resolved' && dispute.resolvedDate && (
                <div className="detail-item">
                  ...
                  <span className="detail-value success">{formatDate(dispute.resolvedDate)}</span>
                </div>
              )}
            </div>
            ...
            <div className="dispute-actions">
              <button 
                ...
                onClick={() => window.location.href = `/credit-repair/disputes/${dispute.id}`}
              >
                View Details
              </button>
              <button 
                className="btn btn-outline btn-sm"
                onClick={() => window.location.href = `/credit-repair/disputes/${dispute.id}/letters`}
              >
                View Letters
              </button>
            </div>
