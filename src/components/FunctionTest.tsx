...
const FunctionTest = () => {
  ...
  const testHealthCheck = async () => {
    ...
    try {
      ...
      setResults(error ? `❌ Error: ${error.message}` : `✅ Health: ${JSON.stringify(data)}`)
    } catch (err) {
  ...
  const testSMS = async () => {
    setLoading(true)
    try {
      const { data, error } = await api.sendSMS('+1234567890', 'Test from RepMotivatedSeller')
      setResults(error ? `❌ SMS Error: ${error.message}` : `✅ SMS: ${JSON.stringify(data)}`)
    } catch (err) {
