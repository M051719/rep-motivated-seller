...
async function getRecentSubmissions(supabase, filters = {}) {
  ...
  if (filters.search) {
    query = query.or(`property_address.ilike.%${filters.search}%,contact_name.ilike.%${filters.search}%`)
  }
...
async function getUpcomingFollowups(supabase, filters = {}) {
  ...
  if (filters.dateRange) {
    query = query.lte('scheduled_date', filters.dateRange.end)
